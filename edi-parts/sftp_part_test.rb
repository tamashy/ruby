...
def connect_sftp(hostname, username, password, port=22, isverbose)
  begin
    log_error("Connecting to remote server #{hostname} by sFTP protocol")
    if isverbose
      puts "Starting SFTP connection in verbose mode"
      sftp = Net::SFTP.start(hostname, username, :password => password, :port => port, :timeout => 20, :keepalive => true, :keepalive_maxcount => 3, :keepalive_interval => 10, :verbose=>:info)
    else  
...
def download_from_sftp(sfd_connection, remote_dir, dir_local, backup_rem_dir, file_regex, isverbose, issingle)
  s_file = []
  ...
  if s_file.empty? == true
    ...
    if isverbose
      ...
      puts "#{full_rem_dir} permissions: %04o" % sfd_connection.lstat!(full_rem_dir).permissions
      puts "#{full_rem_back_dir} permissions: %04o" % sfd_connection.lstat!(full_rem_back_dir).permissions
    end
